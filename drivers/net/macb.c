...
static int macb_eth_probe(struct udevice *dev)
{
	...
	/* RP1-specific initialization for Raspberry Pi 5 */
	if (device_is_compatible(dev, "raspberrypi,rp1-macb")) {
		ret = rp1_macb_init(macb->regs);
		if (ret) {
			dev_err(dev, "RP1 MACB initialization failed: %d\n", ret);
			return ret;
		}
		dev_dbg(dev, "RP1 MACB initialization complete\n");
	}
...
/* RP1 (Raspberry Pi 5) configuration */
static const struct macb_config rp1_config = {
	.dma_burst_length = RP1_MACB_DMA_BURST_LENGTH,
	.hw_dma_cap = HW_DMA_CAP_32B,
	.clk_init = NULL,
	.usrio = &macb_default_usrio,
};
...
static const struct udevice_id macb_eth_ids[] = {
	...
	{ .compatible = "raspberrypi,rp1-macb",
	  .data = (ulong)&rp1_config },
	{ }
};
